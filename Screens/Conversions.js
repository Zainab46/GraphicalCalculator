...
function ConversionsMenu({navigation}){

  const ConversionsMenuItems=(id,value)=>{
   if(id=='1'){
  ...
  return (
    <View style={styles.container}>
      <FlatList
        data={modes.conversions}
        keyExtractor={(item) => item.id}
        renderItem={({ item }) => (
            <View style={{width:'100%',borderRadius:5,borderWidth:1,backgroundColor:'#434547',borderColor:'#83888d',flex:1}}>

           <TouchableOpacity onPress={() => ConversionsMenuItems(item.id,item.value)} style={styles.item}>
