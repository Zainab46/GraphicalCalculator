...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, ScrollView,Button,RadioButton } from 'react-native';
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <RadioButton.Group onValueChange={setDimension} value={dimension}>
      ...
      <View style={styles.operations}>
        {['add', 'subtract', 'dot', 'cross', 'magnitudeA', 'magnitudeB'].map(op => (
          <TouchableOpacity key={op} onPress={() => setOperation(op)} style={[
            styles.opButton,
            operation === op && styles.opButtonActive
          ]}>
...
const styles = StyleSheet.create({
  ...
  input: {
    borderWidth: 1, borderColor: '#ccc', padding: 8, marginHorizontal: 5,
    borderRadius: 6, width: 60, backgroundColor: '#fff'
  },
  radioGroup: { flexDirection: 'row' },
  operations: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginVertical: 10 },
  opButton: {
    backgroundColor: '#eee', padding: 10, borderRadius: 8, minWidth: 70, alignItems: 'center'
  },
  opButtonActive: { backgroundColor: '#6200ee' },
  opText: { color: '#000' },
  calcButton: { marginTop: 20, backgroundColor: '#6200ee' },
  resultLabel: { marginTop: 20, fontSize: 16, fontWeight: 'bold' },
  result: { fontSize: 18, marginTop: 10, backgroundColor: '#fff', padding: 10, borderRadius: 6 }
});
